...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (req.url !== environment.authUrl) { // NOTE: consider using a whitelist of sites to send the token to
      // ...then check to see if we are logged in.
      if (this.isLoggedIn) {
        // If we are, then add the token to the Authorization header and send that.
        const newHeaders = req.headers.append('Authorization', 'Bearer ' + this.token);
        const authReq = req.clone({ headers: newHeaders });
        return next.handle(authReq);
      }
    } else {
      // ...otherwise don't do anything, just hand it off
